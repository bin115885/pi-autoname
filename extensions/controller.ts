import type { AutonameConfig, DialoguePart, RenameMarker } from "./lib.ts";
...
export function createNamingController(runtime: NamingControllerRuntime): NamingController {
  ...
  const applyResult = (result: NamingResult, sequence: number): NamingResult | undefined => {
    ...
    // Pi emits session_info_changed from setSessionName(). Remember ownership
    // before writing so the event cannot be mistaken for a user rename.
    lastGeneratedName = name;
    manualName = undefined;
    if (name !== currentName) {
      runtime.setSessionName(name);
      runtime.debug(`session name updated: ${currentName ?? "(none)"} -> ${name}`);
    } else {
  ...
  const rename = async (mode: NamingMode): Promise<NamingResult | undefined> => {
    ...
    try {
      ...
      if (!controller.signal.aborted) {
        runtime.debug(`naming request failed: ${error instanceof Error ? error.message : String(error)}`);
      }
      return undefined;
    } finally {
  ...
  return {
    ...
    async handleSettled() {
      ...
      if (runtime.now() - lastRenameTime < cooldownMs) return;
      await rename("periodic");
    },
