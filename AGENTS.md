...
## 项目一句话定位
Pi 扩展：AI 驱动的会话语义自动命名。首轮对话后自动命名、每 10 分钟静默重命名、`/autoname` 手动触发。npm 包 `pi-autoname`。

## 先读文档顺序
1. 本文件
2. `README.md`（功能、安装、配置项）
...
## 代码边界与安全边界
- 不硬编码 API Key / 模型配置
- 调试日志默认关闭，只在 debug 模式显示

## 验证方式
`pi install` 后加载测试；`/autoname` + `/reload`
